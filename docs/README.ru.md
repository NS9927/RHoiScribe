...
Пути являются относительными к моду. Небезопасные пути, пути с drive prefix и directory traversal отклоняются до записи.

## Форма проекта

RHoiScribe построен на Rust с `rmcp` и использует stdio transport. Runtime resources встроены из репозитория, поэтому настроенный клиент может читать prompts, resources и tools без live network access.

Текущая форма релиза практична: сначала agent-facing knowledge base и batch generators для распространенных HOI4 files, затем расширение к более глубокой semantic validation и более богатым генераторам для полного mod production workflow.
